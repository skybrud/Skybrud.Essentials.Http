...
public partial class OAuthClient {
    ...
    public virtual string GenerateSignature(HttpMethod method, string url, NameValueCollection? queryString, NameValueCollection? body) {
    ...
    public virtual string GenerateParameterString(NameValueCollection? queryString, NameValueCollection? body) {
        ...
        if (string.IsNullOrWhiteSpace(ConsumerKey)) throw new PropertyNotSetException(nameof(ConsumerKey));
        if (string.IsNullOrWhiteSpace(Nonce)) throw new PropertyNotSetException(nameof(Nonce));
        if (string.IsNullOrWhiteSpace(Timestamp)) throw new PropertyNotSetException(nameof(Timestamp));
        ...
        if (queryString != null) {
            foreach (string key in queryString.Keys) {
                ...
                if (queryString[key] is { } value) sorted.Add(Uri.EscapeDataString(key), Uri.EscapeDataString(value));
            }
        }
        ...
        if (body != null) {
            foreach (string key in body.Keys) {
                if (key is null) continue;
                if (body[key] is { } value) sorted.Add(Uri.EscapeDataString(key), Uri.EscapeDataString(value));
            }
        }
